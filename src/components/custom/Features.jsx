const FEATURES = [
  {
    ...
    icon: (
      <svg
        viewBox="0 0 24 24"
        className="w-[14px] h-[14px] stroke-stone-500 fill-none"
        strokeWidth={1.6}
      >
...
export default function Features() {
  return (
    <section id="features" className="max-w-4xl mx-auto px-10 pb-16">
      {/* Heading */}
      ...
      <h2 className="font-serif text-[36px] font-normal text-stone-900 leading-[1.2] mb-3">
        What you get
        <br />
        <i className="text-stone-400">in every trip.</i>
      </h2>
      ...
      {/* Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 border border-stone-200 rounded-xl overflow-hidden divide-y md:divide-y divide-stone-200">
        {FEATURES.map(({ title, description, icon }, i) => (
          <div
            key={title}
            className={`bg-stone-50 p-6 ${
              i % 2 === 0 ? "md:border-r border-stone-200" : ""
            }`}
          >
            <div className="w-[32px] h-[32px] flex items-center justify-center border border-stone-200 rounded-[7px] mb-3">
              {icon}
            </div>
